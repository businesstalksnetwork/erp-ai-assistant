...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: fileData, error: dlError } = await supabase.storage.from("legacy-imports").download("dbo.A_UnosPodataka.csv");
    if (dlError) throw new Error(`Storage download error: ${dlError.message}`);

    const csvText = await fileData.text();
    const lines = csvText.split("\n").filter((l) => l.trim().length > 0);

    const { data: existingSkus } = await supabase.from("products").select("sku").eq("tenant_id", TENANT_ID);
    ...
    for (const line of lines) {
      ...
      batch.push({
        tenant_id: TENANT_ID, sku, name, unit_of_measure: unitOfMeasure,
    ...
    return createErrorResponse(err, req, { logPrefix: "import-legacy-products" });
  }
});
