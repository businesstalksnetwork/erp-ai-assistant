...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: "Invalid or empty request body" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    const { tenant_id, language = "en", time_of_day } = body;
    if (!tenant_id) {
    ...
    return createErrorResponse(e, req, { logPrefix: "ai-daily-digest error" });
  }
});
