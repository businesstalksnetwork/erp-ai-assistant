...
-- Migration 1: Update calculate_payroll_for_run to use payroll_parameters + contribution base clamping
...
BEGIN
  ...
  -- Fetch effective payroll parameters for this tenant/period
  SELECT * INTO v_params FROM payroll_parameters
    WHERE tenant_id = v_run.tenant_id AND effective_from <= v_period_start
    ORDER BY effective_from DESC LIMIT 1;

  -- Fallback to statutory defaults if no parameters configured
  IF NOT FOUND THEN
    v_params := ROW(
      NULL, v_run.tenant_id, v_period_start,
      0.14, 0.0515, 0.0075, 0.10,
      0.12, 0.0515,
      34221, 51297, 732820,
      NULL, NULL
    )::payroll_parameters;
  ...
  FOR v_contract IN
    SELECT ec.*, e.id as emp_id, COALESCE(ec.work_hours_per_week, 40) as wk_hours
  ...
  LOOP
    ...
    -- Contribution base clamping (proportional for part-time)
    ...
    -- Employee contributions (on clamped base)
    ...
    v_unemp_e := ROUND(v_contrib_base * COALESCE(v_params.unemployment_rate, 0.0075), 2);

    -- Tax base (on gross, not clamped base)
    ...
    -- Employer contributions (on clamped base)
    v_pio_r := ROUND(v_contrib_base * COALESCE(v_params.pio_employer_rate, 0.12), 2);
  ...
  UPDATE payroll_runs SET status='calculated', total_gross=v_tg, total_net=v_tn, total_taxes=v_tt, total_contributions=v_tc
