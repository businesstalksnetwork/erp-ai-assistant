...
-- Step 2: eBolovanje → leave_requests sync bridge
-- When ebolovanje_claims status changes to 'confirmed', auto-create a leave_requests record

CREATE OR REPLACE FUNCTION public.sync_ebolovanje_to_leave_requests()
RETURNS TRIGGER AS $$
DECLARE
  v_days int;
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS DISTINCT FROM 'confirmed') THEN
    ...
    -- Check if a leave_request already exists for this claim
    IF NOT EXISTS (
      ...
      INSERT INTO leave_requests (
        tenant_id, employee_id, leave_type, start_date, end_date, days, status, notes
      ) VALUES (
        NEW.tenant_id, NEW.employee_id, 'sick_leave', NEW.start_date, NEW.end_date,
        v_days, 'approved',
        'Auto-created from eBolovanje claim #' || COALESCE(NEW.rfzo_claim_number, NEW.id::text)
      );
    END IF;
  END IF;
...
DROP TRIGGER IF EXISTS trg_sync_ebolovanje_to_leave_requests ON ebolovanje_claims;
CREATE TRIGGER trg_sync_ebolovanje_to_leave_requests
  AFTER UPDATE ON ebolovanje_claims
  FOR EACH ROW
  EXECUTE FUNCTION sync_ebolovanje_to_leave_requests();

-- Also fire on insert if status is already 'confirmed'
DROP TRIGGER IF EXISTS trg_sync_ebolovanje_to_leave_requests_insert ON ebolovanje_claims;
CREATE TRIGGER trg_sync_ebolovanje_to_leave_requests_insert
  AFTER INSERT ON ebolovanje_claims
  FOR EACH ROW
  WHEN (NEW.status = 'confirmed')
  EXECUTE FUNCTION sync_ebolovanje_to_leave_requests();

-- Step 3: Minuli rad columns on payroll_items
ALTER TABLE payroll_items ADD COLUMN IF NOT EXISTS minuli_rad_years numeric DEFAULT 0;
ALTER TABLE payroll_items ADD COLUMN IF NOT EXISTS minuli_rad_amount numeric DEFAULT 0;

-- Step 3: Update calculate_payroll_for_run to include minuli rad
