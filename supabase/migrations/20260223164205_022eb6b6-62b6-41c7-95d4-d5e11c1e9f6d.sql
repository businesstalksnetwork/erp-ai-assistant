...
-- Fix 1: Update calculate_payroll_for_run() with 2026 Serbian legal values
...
BEGIN
  ...
  IF NOT FOUND THEN
    -- 2026 Serbian legal defaults (Sl. glasnik RS)
...
-- Insert 2026 payroll parameters for all existing tenants that don't have them yet
INSERT INTO payroll_parameters (tenant_id, effective_from, tax_rate, nontaxable_amount, min_contribution_base, max_contribution_base, pio_employee_rate, pio_employer_rate, health_employee_rate, health_employer_rate, unemployment_employee_rate)
SELECT t.id, '2026-01-01', 0.10, 34221, 51297, 732820, 0.14, 0.11, 0.0515, 0.0515, 0.0075
FROM tenants t
WHERE NOT EXISTS (
  SELECT 1 FROM payroll_parameters pp 
  WHERE pp.tenant_id = t.id AND pp.effective_from = '2026-01-01'
);
