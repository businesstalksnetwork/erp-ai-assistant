...
-- =============================================
-- Step 1: Add CIT (Corporate Income Tax) accrual to year-end closing
-- Serbian law: 15% CIT on positive net income
-- Accounts: 7200 (Tax Expense) debit, 4810 (CIT Payable) credit
-- =============================================
...
DECLARE
  ...
  v_uid UUID;
  -- CIT variables
  v_cit_rate NUMERIC := 0.15;  -- 15% Serbian CIT rate
  v_cit_amount NUMERIC := 0;
  v_tax_expense_id UUID;
  v_cit_payable_id UUID;
BEGIN
  ...
  -- CIT Accrual: if net income is positive, accrue 15% Corporate Income Tax
  IF v_net_income > 0 THEN
    -- Look up CIT accounts
    SELECT id INTO v_tax_expense_id FROM public.chart_of_accounts
    WHERE tenant_id = p_tenant_id AND code = '7200' AND is_active = true LIMIT 1;

    SELECT id INTO v_cit_payable_id FROM public.chart_of_accounts
    WHERE tenant_id = p_tenant_id AND code = '4810' AND is_active = true LIMIT 1;

    IF v_tax_expense_id IS NOT NULL AND v_cit_payable_id IS NOT NULL THEN
      v_cit_amount := ROUND(v_net_income * v_cit_rate, 2);

      -- Debit 7200 Tax Expense
      v_line_order := v_line_order + 1;
      INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, sort_order)
      VALUES (v_entry_id, v_tax_expense_id, v_cit_amount, 0, 'CIT accrual 15% on net income', v_line_order);

      -- Credit 4810 CIT Payable
      v_line_order := v_line_order + 1;
      INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, sort_order)
      VALUES (v_entry_id, v_cit_payable_id, 0, v_cit_amount, 'CIT payable 15% on net income', v_line_order);

      -- Reduce net income by CIT for retained earnings
      v_net_income := v_net_income - v_cit_amount;
    ELSE
      RAISE NOTICE 'CIT accounts (7200/4810) not found — skipping CIT accrual';
    END IF;
  END IF;

  -- Post to retained earnings (net of CIT)
  v_line_order := v_line_order + 1;
  IF v_net_income >= 0 THEN
    INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, sort_order)
    VALUES (v_entry_id, v_retained_earnings_id, 0, v_net_income, 'Net income to retained earnings (after CIT)', v_line_order);
  ELSE
    INSERT INTO public.journal_lines (journal_entry_id, account_id, debit, credit, description, sort_order)
