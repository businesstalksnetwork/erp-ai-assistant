...
-- P1-05: Add superseded_at IS NULL filter to BOM lines query
-- P3-03: Fix production GL fallback accounts (5100→1200, 5000→5110)
...
BEGIN
  ...
  IF v_order.bom_template_id IS NOT NULL THEN
    FOR v_bom_line IN
      ...
      WHERE bl.bom_template_id = v_order.bom_template_id
        AND bl.superseded_at IS NULL  -- P1-05: Only consume active BOM lines
    LOOP
  ...
  IF v_total_material_cost > 0 THEN
    ...
    -- P3-03: Fixed fallback accounts: 1200 (Gotovi proizvodi) and 5110 (Troškovi materijala)
    IF v_a_finished IS NULL THEN SELECT id INTO v_a_finished FROM chart_of_accounts WHERE tenant_id = v_order.tenant_id AND code = '1200' AND is_active LIMIT 1; END IF;
    IF v_a_wip IS NULL THEN SELECT id INTO v_a_wip FROM chart_of_accounts WHERE tenant_id = v_order.tenant_id AND code = '5110' AND is_active LIMIT 1; END IF;
    IF v_a_finished IS NULL OR v_a_wip IS NULL THEN RAISE EXCEPTION 'Missing accounts for production posting (1200/5110)'; END IF;
