...
-- CR2-01: Drop the wrongly-named function so only calculate_payroll_for_run exists
DROP FUNCTION IF EXISTS public.calculate_payroll(uuid);

-- CR2-01/02/03/06/07: Recreate with correct name + all fixes
...
BEGIN
  ...
  -- CR2-03: Calculate period dates
  v_period_start := (v_run.period_year || '-' || LPAD(v_run.period_month::TEXT, 2, '0') || '-01')::DATE;
  ...
  -- CR2-03: Dynamic weekday count for the month (excluding Sat/Sun)
  SELECT COUNT(*)::INTEGER INTO v_wd
  ...
  LOOP
    ...
    SELECT COUNT(*)::INTEGER INTO v_ad FROM attendance_records
      WHERE employee_id = v_contract.emp_id AND tenant_id = v_run.tenant_id
        AND date >= v_period_start AND date < v_period_start + INTERVAL '1 month'
        AND status = 'present';
    ...
    -- CR2-06: Count actual overlapping DAYS for unpaid leave (not request count)
    SELECT COALESCE(SUM(
      -- Count weekdays in the overlap between leave period and payroll period
      (SELECT COUNT(*)::INTEGER FROM generate_series(
        GREATEST(lr.start_date, v_period_start),
        LEAST(lr.end_date, v_period_end),
        '1 day'::INTERVAL
      ) d WHERE EXTRACT(DOW FROM d) NOT IN (0, 6))
    ), 0)::INTEGER INTO v_ld_cnt
    FROM leave_requests lr
    WHERE lr.employee_id = v_contract.emp_id AND lr.tenant_id = v_run.tenant_id
      AND lr.status = 'approved'
      AND lr.leave_type = 'unpaid'
      AND lr.start_date <= v_period_end
      AND lr.end_date >= v_period_start;

    -- CR2-07: Sick leave from leave_requests (not attendance_records)
    SELECT COALESCE(SUM(
      (SELECT COUNT(*)::INTEGER FROM generate_series(
        GREATEST(lr.start_date, v_period_start),
        LEAST(lr.end_date, v_period_end),
    ...
    ), 0)::INTEGER INTO v_employer_sick_days
    FROM leave_requests lr
    WHERE lr.employee_id = v_contract.emp_id AND lr.tenant_id = v_run.tenant_id
      AND lr.status = 'approved'
    ...
    -- Tax base: gross minus employee contributions minus nontaxable amount
    ...
    -- CR2-02: Removed duplicate pension_employer/health_employer columns
