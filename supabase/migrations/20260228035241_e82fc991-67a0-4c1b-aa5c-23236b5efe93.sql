...
-- ============================================================
-- Phase 2 Deferred Fixes: Payroll RPC, Tax Depreciation, Journal Numbering
-- ============================================================

-- P2-01: Fix payroll tax base formula per ZPDG Art. 15a
-- Employee contributions (PIO+Health+Unemp) MUST be deducted before calculating income tax
-- P2-04: Fix unpaid leave to count DAYS not requests
-- P2-05: Fix employer PIO rate fallback from 10% to 12% per ZDSO Art. 44
...
BEGIN
  ...
  FOR v_contract IN
    SELECT ec.*, e.id as emp_id, e.first_name, e.last_name,
           ...
           e.start_date as emp_start_date, e.hire_date as emp_hire_date
  ...
  LOOP
    ...
    -- Minuli rad (seniority bonus): 0.4% per year of service (Zakon o radu čl. 108)
    v_emp_start_date := COALESCE(v_contract.emp_hire_date, v_contract.emp_start_date);
    v_minuli_rad_years := 0;
    v_minuli_rad_amount := 0;
    IF v_emp_start_date IS NOT NULL AND v_emp_start_date < v_period_start THEN
      v_minuli_rad_years := ROUND(EXTRACT(EPOCH FROM (v_period_start - v_emp_start_date)) / (365.25 * 86400), 1);
      v_minuli_rad_amount := ROUND(0.004 * v_minuli_rad_years * v_gross, 2);
    ...
    -- P2-04: Unpaid leave — count actual DAYS overlapping the period, not COUNT of requests
    SELECT COALESCE(SUM(
      (LEAST(end_date, v_period_end) - GREATEST(start_date, v_period_start))::int + 1
    ), 0) INTO v_ld_cnt
    FROM leave_requests
    ...
    -- P2-01: FIXED per ZPDG Art. 15a — deduct employee contributions from gross BEFORE calculating tax base
    -- Old (WRONG): v_taxbase := GREATEST(v_gross - nontaxable, 0)
    -- New (CORRECT): v_taxbase := GREATEST(v_gross - v_pio_e - v_health_e - v_unemp_e - nontaxable, 0)
    v_taxbase := GREATEST(v_gross - v_pio_e - v_health_e - v_unemp_e - COALESCE(v_params.nontaxable_amount, 28423), 0);
...
-- ============================================================
-- P2-03: Fix ZPDP Tax Depreciation — Annual calculation per ZPDP Art. 10
-- Remove / 12.0 division; ZPDP is annual for PB-1 tax return
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    -- P2-03 FIXED: Annual declining balance per ZPDP Art. 10 — NO monthly division
    -- Formula: (cost - accumulated) * rate / 100
    ROUND(
      GREATEST(0,
...
-- ============================================================
-- P2-10: Atomic server-side journal entry number generation
-- Replaces client-side COUNT-based approach with server-side RPC using advisory lock
-- ============================================================
...
BEGIN
  ...
  -- Use advisory lock based on tenant_id hash to prevent race conditions
  v_lock_id := abs(hashtext(p_tenant_id::text || '_journal_seq'));
  PERFORM pg_advisory_xact_lock(v_lock_id);
  
  -- Get prefix from tenant_settings
  SELECT COALESCE((settings->>'journal_prefix')::TEXT, 'JE')
  INTO v_prefix
  FROM tenant_settings
  WHERE tenant_id = p_tenant_id;
  
  IF v_prefix IS NULL THEN v_prefix := 'JE'; END IF;
  
  -- Count existing entries for the year + 1
  SELECT COALESCE(COUNT(*), 0) + 1
  INTO v_next_seq
  FROM journal_entries
  WHERE tenant_id = p_tenant_id
    AND entry_date >= make_date(v_year, 1, 1)
    AND entry_date <= make_date(v_year, 12, 31);
  
  RETURN v_prefix || '-' || v_year::TEXT || '-' || LPAD(v_next_seq::TEXT, 5, '0');
END;
$$;
