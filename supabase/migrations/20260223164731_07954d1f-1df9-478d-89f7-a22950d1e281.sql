...
-- Add minimum wage validation to calculate_payroll_for_run()
...
BEGIN
  ...
  FOR v_contract IN
    SELECT ec.*, e.id as emp_id, e.first_name, e.last_name, COALESCE(ec.working_hours_per_week, 40) as wk_hours
  ...
  LOOP
    ...
    -- Minimum wage validation: monthly fund = weekly_hours / 5 * working_days
    v_monthly_fund_hours := (v_work_hours / 5.0) * v_wd;
    -- Min wage is NET per hour, approximate gross minimum = net_hourly * hours / (1 - tax_and_contrib_rates)
    -- Simplified: just validate that gross >= min_hourly * monthly_hours (conservative check)
    v_min_gross := v_min_hourly_wage * v_monthly_fund_hours;
    IF v_gross < v_min_gross THEN
      ...
      -- Don't block, but log warning. Actual enforcement should be at contract level.
    END IF;
