-- Add payment_type_id to work_logs for optional explicit payment type linkage
ALTER TABLE work_logs ADD COLUMN IF NOT EXISTS payment_type_id UUID REFERENCES payroll_payment_types(id);

-- Create a mapping table from work_log type strings to payment type codes
CREATE TABLE IF NOT EXISTS work_log_payment_type_map (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tenant_id UUID NOT NULL REFERENCES tenants(id),
  work_log_type TEXT NOT NULL,
  payment_type_code TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE(tenant_id, work_log_type)
);

ALTER TABLE work_log_payment_type_map ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Tenant isolation" ON work_log_payment_type_map
  FOR ALL USING (tenant_id IN (SELECT get_user_tenant_ids(auth.uid())));

-- Seed default mappings for all tenants
INSERT INTO work_log_payment_type_map (tenant_id, work_log_type, payment_type_code)
SELECT DISTINCT t.id, v.wl_type, v.pt_code
FROM tenants t
CROSS JOIN (VALUES
  ('workday', '100'),
  ('weekend', '107'),
  ...
  ('maternity_leave', '105'),
  ('slava', '110')
) AS v(wl_type, pt_code)
WHERE NOT EXISTS (
  SELECT 1 FROM work_log_payment_type_map m WHERE m.tenant_id = t.id AND m.work_log_type = v.wl_type
);

-- Upgrade the payroll calculation RPC to use payment type multipliers from work_logs
