...
function trendTopColor(trend?: number): string {
  if (trend === undefined || trend === 0) return "border-t-border";
  ...
  return (
    <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 xl:grid-cols-4">
      {stats.map((stat) => (
        <Card
          key={stat.label}
          className={`overflow-hidden border-t-2 ${trendTopColor(stat.trend)} hover:shadow-md`}
        >
            ...
            <div className="mt-1.5 flex items-end justify-between gap-2">
              ...
              {stat.sparklineData && stat.sparklineData.length >= 2 && (
                <MiniSparkline
                  ...
                  color={
                    stat.trend !== undefined && stat.trend !== 0
                      ? stat.trend > 0 ? "hsl(var(--accent))" : "hsl(var(--destructive))"
                      : "hsl(var(--primary))"
                  }
