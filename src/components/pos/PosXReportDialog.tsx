...
export function PosXReportDialog({ open, onOpenChange, sessionId }: PosXReportDialogProps) {
  ...
  const { data: report, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { salesCount: sales.length, totalSales, refundsCount: refunds.length, totalRefunds, netTotal: totalSales - totalRefunds, totalTax, byMethod, cashTotal };
    },
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            X-{t("report") || "Izveštaj"} ({t("interim") || "Privremeni"})
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <Card>
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-muted-foreground">{t("sales") || "Prodaje"}</p>
              ...
              <Card>
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-muted-foreground">{t("refunds") || "Povraćaji"}</p>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">{t("byPaymentMethod") || "Po načinu plaćanja"}</CardTitle>
              </CardHeader>
            ...
            <div className="flex justify-between font-bold text-lg border-t pt-3">
              <span>{t("netTotal") || "Neto ukupno"}:</span>
            ...
            <Card className="bg-muted/50">
              <CardContent className="p-3 space-y-2">
                <p className="text-sm font-medium flex items-center gap-2">
                  ...
                  {t("cashCount") || "Prebrojavanje gotovine"}
                </p>
                <div className="flex items-center gap-3">
                  <span className="text-sm text-muted-foreground">{t("expectedCash") || "Očekivano"}:</span>
                  <span className="font-mono font-bold">{report.cashTotal.toFixed(2)}</span>
                </div>
                <Input
                  type="number"
                  step="0.01"
                  placeholder={t("actualCashAmount") || "Stvarni iznos gotovine..."}
                  value={cashCount}
        ...
        <DialogFooter>
          <p className="text-xs text-muted-foreground mr-auto">
            {t("xReportNote") || "* X-izveštaj ne zatvara smenu"}
          </p>
