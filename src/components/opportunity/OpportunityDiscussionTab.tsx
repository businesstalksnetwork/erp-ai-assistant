          ...
          {isLoading ? (
            ...
            comments.map((c: any) => (
              <div key={c.id} className={`flex gap-3 ${c.user_id === user?.id ? "flex-row-reverse" : ""}`}>
                ...
                <div className={`max-w-[75%] rounded-lg p-3 text-sm ${c.user_id === user?.id ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
                  ...
                  <p className="whitespace-pre-wrap">{renderContent(c.content)}</p>
                </div>
              </div>
            ))
          )}
