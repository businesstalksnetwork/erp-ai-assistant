...
export default function WithholdingTax() {
  ...
  const createMut = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("withholding_tax").insert({
        ...
        treaty_applied: form.treaty_applied,
  ...
  const statusLabels: Record<string, string> = { draft: "Nacrt", calculated: "Obračunat", paid: "Plaćen", reported: "Prijavljen" };
  const incomeTypes: Record<string, string> = {
    services: "Usluge",
    royalties: "Autorska prava",
    interest: "Kamata",
    dividends: "Dividende",
    capital_gains: "Kapitalni dobici",
    other: "Ostalo",
  };
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <PageHeader
        title="Porez po odbitku"
        description="Obračun poreza po odbitku na isplate nerezidentima (20% ili umanjena stopa po ugovoru)"
      />

      <div className="flex justify-between items-center">
        <p className="text-sm text-muted-foreground">
          Ukupan porez po odbitku: <span className="font-semibold">{totalTax.toLocaleString("sr-RS")} RSD</span>
        </p>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base flex items-center gap-2"><Calculator className="h-4 w-4" /> Obračuni</CardTitle></CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-muted-foreground text-sm">Učitavanje...</p>
          ) : records.length === 0 ? (
            <p className="text-muted-foreground text-sm">Nema obračuna.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {records.map((r: any) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>{r.treaty_applied ? <Badge variant="outline">Da</Badge> : "Ne"}</TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Novi obračun poreza po odbitku</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Bruto iznos (RSD)</Label>
                <Input type="number" min="0" step="0.01" value={form.gross_amount} onChange={(e) => setForm({ ...form, gross_amount: e.target.value })} />
              </div>
              <div>
                <Label>Zakonska stopa (%)</Label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={form.income_type} onValueChange={(v) => setForm({ ...form, income_type: v })}>
                  ...
                  <SelectContent>
                    {Object.entries(incomeTypes).map(([k, v]) => (
              ...
              <div>
                <Label>Zemlja (ISO kod)</Label>
                <Input value={form.country_code} onChange={(e) => setForm({ ...form, country_code: e.target.value.toUpperCase() })} placeholder="npr. DE, US, GB" maxLength={2} />
              </div>
            </div>
            {form.gross_amount && (
              <Card className="bg-muted/50">
                <CardContent className="p-3 grid grid-cols-3 text-center">
                  <div><p className="text-xs text-muted-foreground">Porez</p><p className="font-semibold">{Math.round(taxAmount).toLocaleString("sr-RS")} RSD</p></div>
                  <div><p className="text-xs text-muted-foreground">Neto</p><p className="font-semibold">{Math.round(netAmount).toLocaleString("sr-RS")} RSD</p></div>
                  <div><p className="text-xs text-muted-foreground">Efektivna stopa</p><p className="font-semibold">{form.tax_rate}%</p></div>
                </CardContent>
              </Card>
            )}
            <div>
