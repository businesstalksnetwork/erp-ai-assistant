...
export default function FleetFuelLog() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Gorivo evidentirano");
  ...
  return (
    <div className="space-y-4 p-1">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Evidencija goriva</h1>
          <p className="text-muted-foreground text-sm">Svi unosi utroška goriva po vozilima</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button><Plus className="h-4 w-4 mr-2" /> Novi unos</Button>
          </DialogTrigger>
          <DialogContent className="max-w-lg">
            <DialogHeader><DialogTitle>Evidencija goriva</DialogTitle></DialogHeader>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Datum</Label><Input type="date" value={form.log_date} onChange={e => set("log_date", e.target.value)} /></div>
                <div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div><Label>Litara</Label><Input type="number" step="0.01" value={form.quantity_liters} onChange={e => set("quantity_liters", e.target.value)} /></div>
                <div><Label>Cena/L</Label><Input type="number" step="0.01" value={form.price_per_liter} onChange={e => set("price_per_liter", e.target.value)} /></div>
                <div><Label>Ukupno (RSD)</Label><Input type="number" step="0.01" value={form.total_cost} onChange={e => set("total_cost", e.target.value)} /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Kilometraža</Label><Input type="number" value={form.odometer_km} onChange={e => set("odometer_km", e.target.value)} /></div>
                <div><Label>Pumpa</Label><Input value={form.station_name} onChange={e => set("station_name", e.target.value)} /></div>
              </div>
              <div><Label>Broj računa</Label><Input value={form.receipt_number} onChange={e => set("receipt_number", e.target.value)} /></div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">Nema unosa</TableCell></TableRow>
              ) : filtered.map((l: any) => (
                <TableRow key={l.id}>
