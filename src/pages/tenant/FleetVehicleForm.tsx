...
export default function FleetVehicleForm() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isEdit) {
        const { error } = await supabase.from("fleet_vehicles").update({
          ...
          seat_count: parseInt(form.seat_count) || 5,
    ...
    onSuccess: () => {
      toast.success(isEdit ? "Vozilo ažurirano" : "Vozilo dodato");
  ...
  return (
    <div className="space-y-4 p-1">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold">{isEdit ? "Izmeni vozilo" : "Novo vozilo"}</h1>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            {!isEdit && (
              <div><Label>Naziv sredstva</Label><Input value={form.asset_name} onChange={e => set("asset_name", e.target.value)} placeholder="npr. Fiat Doblo BG-123-AB" /></div>
            )}
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Registarska oznaka</Label><Input value={form.registration_plate} onChange={e => set("registration_plate", e.target.value)} placeholder="BG-123-AB" /></div>
              <div><Label>VIN</Label><Input value={form.vin} onChange={e => set("vin", e.target.value)} /></div>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div><Label>Marka</Label><Input value={form.make} onChange={e => set("make", e.target.value)} placeholder="Fiat" /></div>
              <div><Label>Model</Label><Input value={form.model} onChange={e => set("model", e.target.value)} placeholder="Doblo" /></div>
              <div><Label>Godina</Label><Input type="number" value={form.year_of_manufacture} onChange={e => set("year_of_manufacture", e.target.value)} /></div>
            </div>
            <div><Label>Boja</Label><Input value={form.color} onChange={e => set("color", e.target.value)} /></div>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle>Tehnički podaci</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                <Label>Klasa vozila</Label>
                <Select value={form.vehicle_class} onValueChange={v => set("vehicle_class", v)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="passenger">Putničko</SelectItem>
                    <SelectItem value="cargo">Teretno</SelectItem>
                    <SelectItem value="special">Specijalno</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div><Label>Zapremina (cc)</Label><Input type="number" value={form.engine_capacity_cc} onChange={e => set("engine_capacity_cc", e.target.value)} /></div>
              <div><Label>Snaga (kW)</Label><Input type="number" value={form.engine_power_kw} onChange={e => set("engine_power_kw", e.target.value)} /></div>
              <div><Label>Broj sedišta</Label><Input type="number" value={form.seat_count} onChange={e => set("seat_count", e.target.value)} /></div>
            </div>
            <div><Label>Kilometraža</Label><Input type="number" value={form.odometer_km} onChange={e => set("odometer_km", e.target.value)} /></div>
            <div><Label>Napomene</Label><Textarea value={form.notes} onChange={e => set("notes", e.target.value)} rows={3} /></div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}>
          <Save className="h-4 w-4 mr-2" /> {saveMutation.isPending ? "Čuvanje..." : "Sačuvaj"}
        </Button>
      </div>
    </div>
  );
