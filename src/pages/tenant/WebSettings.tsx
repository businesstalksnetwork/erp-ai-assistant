...
import { Plus, Pencil, Trash2, Globe, ShoppingBag, Code, Wifi, WifiOff, RefreshCw } from "lucide-react";
...
export default function WebSettings() {
  ...
  const updateField = (key: keyof ConnectionForm, value: any) =>
    setForm((f) => ({ ...f, [key]: value }));

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">{t("webSales")}</h1>
              ...
              <Card key={conn.id}>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  ...
                  <Badge variant={conn.is_active ? "default" : "secondary"}>
                    {conn.is_active ? (
                      <><Wifi className="h-3 w-3 mr-1" />{t("active")}</>
                    ) : (
                      <><WifiOff className="h-3 w-3 mr-1" />{t("inactive")}</>
                    )}
                  </Badge>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  {conn.last_sync_at && (
                    <p className="text-xs text-muted-foreground">
                      {t("lastSync")}: {new Date(conn.last_sync_at).toLocaleString()}
                    </p>
                  )}
                  {conn.last_error && (
      ...
      <Dialog open={dialog} onOpenChange={setDialog}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {editing ? t("editConnection") : t("addConnection")}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            {form.platform === "shopify" && (
              <>
                <div>
                  <Label>API Key</Label>
                  <Input value={form.api_key} onChange={(e) => updateField("api_key", e.target.value)} />
                </div>
                <div>
                  <Label>{t("accessToken")}</Label>
                  <Input value={form.access_token} onChange={(e) => updateField("access_token", e.target.value)} type="password" />
                </div>
              </>
            )}
            ...
            {form.platform === "woocommerce" && (
              <>
                <div>
                  <Label>{t("consumerKey")}</Label>
                  <Input value={form.api_key} onChange={(e) => updateField("api_key", e.target.value)} placeholder="ck_..." />
                </div>
                <div>
                  <Label>{t("consumerSecret")}</Label>
                  <Input value={form.api_secret} onChange={(e) => updateField("api_secret", e.target.value)} type="password" placeholder="cs_..." />
                </div>
              </>
            )}
            ...
            {form.platform === "custom" && (
              <>
                ...
                <div>
                  ...
                  <Input value={form.webhook_secret} onChange={(e) => updateField("webhook_secret", e.target.value)} type="password" />
                </div>
              </>
            )}
